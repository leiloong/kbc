...
public:
  ...
  Optimizer(
    ...
    for(int i = LEFT; i <= RIGHT; i++) {
      ids[i] = CPU(kLong).zeros({0}).toBackend(model->backend);
      inputs[i] = CPU(kFloat).zeros({0}).toBackend(model->backend);
      grads[i] = CPU(kFloat).zeros({0}).toBackend(model->backend);
      gradient = CPU(kFloat).zeros(
        {model->getNumParameters()}
      ).toBackend(model->backend);
    }
