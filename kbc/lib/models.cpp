...
float Model::getOneScore(uint64_t lhs, uint64_t rel, uint64_t rhs){
  ...
  Tensor output = CPU(kFloat).zeros({1, 1}).toBackend(backend);
...
void Model::getAllScores(float* cpu_output) {
  ...
  Tensor examples = CPU(kLong).zeros({n_examples, 3});
  ...
  Tensor output = CPU(kFloat).zeros(
    {n_examples, shape[RIGHT]}
  ).toBackend(backend);
  Tensor batch[3];
  ...
  Tensor cpu_out = CPU(kFloat).tensor(
    *out_store, 0, {n_examples, shape[RIGHT]});
...
void Model::getRanking(
  ...
  for(int bb = 0; bb < n_examples; bb += kMaxBatchSize) {
    ...
    forward(batch, output, big);
    // compute ranks on CPU
    Tensor cpu_output = output.toBackend(kCPU);
    auto accessor = cpu_output.accessor<float, 2>();
    for(long int b = bb; b <  be; b++) {
      ...
      for(size_t i = 0; i < (size_t) shape[big]; i++) {
        ...
        if(to_skip.size() > 0) {
          ...
          if (ptr_to_skip < skipping.size() and skipping[ptr_to_skip] == i) {
            ptr_to_skip++;
          } else if (accessor[b_out][i] >= accessor[b_out][to_rank]) {
            ranks[b]++;
          }
        } else {
          if (accessor[b_out][i] >= accessor[b_out][to_rank]) ranks[b]++;
        }
      }
    }
  }
...
vector<float> Model::getScores(uint64_t* examples_c, long int n_examples) {
  ...
  Tensor output = CPU(kFloat).zeros({n_examples, 1}).toBackend(backend);
  Tensor batch[3];
...
void Model::learnProbabilities(
  ...
  for (int f = LEFT; f <= RIGHT; f++) {
    probas[f] = CPU(kFloat).zeros(shape[f]);
  }
